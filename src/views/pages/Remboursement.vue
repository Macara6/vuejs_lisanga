    ...
    <div class="w-full max-w-3xl">
      <div class="bg-white dark:bg-gray-800 p-6 sm:p-10 rounded-3xl shadow-xl">
        ...
        <div class="text-center mb-8">
          <h2 class="text-2xl sm:text-3xl font-bold text-gray-800 dark:text-gray-100">💰 Remboursement</h2>
          <p class="text-gray-500 dark:text-gray-400 mt-2 text-sm sm:text-base">
            Visualisez le compte avant le remboursement
          </p>
        </div>
        ...
        <div v-else-if="step === 2" class="space-y-4">
          ...
          <h3 class="font-medium mt-4">Crédits de {{ targetUser.first_name }} :</h3>
          <ul class="space-y-2">
            <li v-for="credit in creditList" :key="credit.id" class="flex items-center space-x-2">
              <input 
                type="radio" 
                :value="credit.id"  
              ...
              />
              <span>
                Crédit : {{ credit.princilal }} USD, Total à rembourser : {{ credit.total_due }} USD, Solde restant : {{ credit.balance_due }} USD
              </span>
            </li>
          </ul>
