...
import { adminDepositSocial, fetchUser } from "@/service/Api";
...
async function submitDeposit(){
    ...
    loading.value =true;
    try{
        const response = await adminDepositSocial(matricule.value, amount.value);
        summary.value = response;
        step.value = 3;
        toast.add({ severity: "success", summary: "Succès", detail: "Dépôt sociale effectué avec succès !" });
    }catch(error){
...
async function fetchUserOrMatricule(){
    ...
    try{
        ...
        if(!user){
            errors.value.matricule = ['Utilisateur introuvable avec ce matrcicule'];
            return false;
        }
  ...
  <div class="w-full md:w-3/4 lg:w-2/3">
    <div class="bg-white dark:bg-gray-800 p-8 rounded-3xl shadow-xl">
      <div class="text-center mb-8">
        <h2 class="text-3xl font-bold text-gray-800 dark:text-gray-100">💰 Dépôt Sociale</h2>
      ...
      <div v-else-if="step === 2" class="space-y-4">
        <div class="bg-gray-50 dark:bg-gray-700 p-4 rounded-xl shadow-inner">
          ...
          <p><b>Solde actuel sociale :</b> {{ targetUser.social }} USD</p>
        </div>
      ...
      <div v-else-if="step === 3" class="space-y-4">
        ...
        <div class="flex justify-start mt-4">
          <Button label="Nouveau dépôt sociale" icon="pi pi-refresh" outlined @click="newDeposit" />
        </div>
      </div>
